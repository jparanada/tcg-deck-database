import { supabase } from "@/lib/supabase";
import Link from "next/link";

interface Format {
  id: string;
  name: string;
  description: string | null;
}

export const dynamic = "force-dynamic";

export default async function HomePage() {
  const { data: formats, error } = await supabase
    .from("formats")
    .select("id, name, description")
    .order("sort_order", { ascending: true });

  if (error) {
    return (
      <div className="max-w-6xl mx-auto px-4 py-12">
        <p className="text-red-500">
          Failed to load formats. Make sure Supabase is configured in .env.local
        </p>
      </div>
    );
  }

  return (
    <div className="max-w-6xl mx-auto px-4 py-12">
      <h1 className="text-3xl font-bold mb-8">Formats</h1>

      {formats.length === 0 ? (
        <div className="text-center py-20">
          <p className="text-gray-500 mb-4">No formats yet.</p>
          <Link
            href="/new"
            className="bg-blue-600 text-white px-6 py-3 rounded-lg font-medium hover:bg-blue-700 transition-colors"
          >
            Create your first format
          </Link>
        </div>
      ) : (
        <>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
            {formats.map((format: Format) => (
              <Link
                key={format.id}
                href={`/format/${format.id}`}
                className="border border-gray-200 dark:border-gray-800 rounded-lg p-5 hover:border-blue-500 transition-colors"
              >
                <h2 className="font-semibold text-lg mb-1">{format.name}</h2>
                {format.description && (
                  <p className="text-sm text-gray-400">{format.description}</p>
                )}
              </Link>
            ))}
          </div>
          <Link
            href="/new"
            className="bg-blue-600 text-white px-6 py-3 rounded-lg font-medium hover:bg-blue-700 transition-colors"
          >
            New Format
          </Link>
        </>
      )}
    </div>
  );
}
